...
端到端测试使用固定离线数据，启动时会写入两只股票、当前快照、每只股票 60 根日 K 和 61 根当日一分钟 K；它不会启动公网采集调度器或后台历史回补任务。每次 Playwright 运行都会新建独立系统临时目录和 DuckDB，进程结束后自动清理，不复用仓库数据，也不依赖上一次测试结果。验收会模拟行情变旧和一分钟柱刷新追加，并验证搜索切股、日/周/月/今日切换、按钮、滚轮、图内拖动和底部缩放条对应的数据窗口变化。
